(() => {
  ...
  const translations = {
    es: {
      ...
      refresh:"Actualizar", platform:"Plataforma", setupStatus:"Preparación", liveCUs:"CUs activas", persistence:"Persistencia al arranque",
    ...
    en: {
      ...
      refresh:"Refresh", platform:"Platform", setupStatus:"Setup", liveCUs:"Live CUs", persistence:"Boot persistence",
  ...
  function requestAction(id) {
    ...
    if (!action) return;
    if (action.risk === "write" || action.risk === "persist" || action.risk === "system") {
      pendingAction = id;
      document.getElementById("confirm-title").textContent = action.risk === "persist" ? tr("confirmPersistTitle") : action.risk === "system" ? tr("confirmSystemTitle") : tr("confirmWriteTitle");
      document.getElementById("confirm-text").textContent = action.risk === "persist" ? tr("confirmPersistText") : action.risk === "system" ? tr("confirmSystemText") : tr("confirmWriteText");
